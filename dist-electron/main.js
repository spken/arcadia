...
async function loadEarlySettings() {
  try {
    const settingsPath = path$3.join(process.env.APP_ROOT, "settings.json");
    const settingsData = await fs$b.readFile(settingsPath, "utf8");
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", async () => {
    ...
    win == null ? void 0 : win.webContents.send("system-info-update", initialSystemInfo);
  });
...
async function getSystemInfo() {
  try {
    ...
    const needsFullUpdate = currentTime - systemInfoCache.lastFullUpdate > 6e4;
    ...
    if (needsFullUpdate || systemInfoCache.totalMemGB === "0") {
      ...
      systemInfoCache.gpuName = gpuData.controllers.length > 0 ? gpuData.controllers[0].name || "N/A" : "N/A";
      ...
      result2.gpu = gpuData.controllers.length > 0 ? `${Math.round(gpuData.controllers[0].utilizationGpu || 0)}%` : "N/A";
      systemInfoCache.gpu = result2.gpu;
      result2.storage = systemInfoCache.storage;
    } else {
      if (currentTime % 3 === 0) {
        const gpuUsage = await si.graphics().then(
          (data) => data.controllers.length > 0 ? Math.round(data.controllers[0].utilizationGpu || 0) : 0
        ).catch(() => 0);
        result2.gpu = `${gpuUsage}%`;
        systemInfoCache.gpu = result2.gpu;
      } else {
        result2.gpu = systemInfoCache.gpu || "N/A";
      }
      result2.storage = systemInfoCache.storage || `0 GB free of ${systemInfoCache.totalStorageGB} GB`;
    }
    result2.memory = `${usedMemGB} GB / ${systemInfoCache.totalMemGB} GB`;
    return result2;
  } catch (error) {
